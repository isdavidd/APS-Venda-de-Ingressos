...
export default function AuthPage() {
  ...
  const formatNome = (value: string) => {
    ...
    if (!regex.test(value)) {
      setNomeError("Nome inválido. Apenas letras, espaços, apóstrofos e hífens são permitidos.");
          ...
          {!isLogin && (
            <>
              <input
                ...
                className={`w-full p-2 border rounded-lg ${nomeError ? 'border-red-500' : ''}`}
                required
              />
              ...
              <input
                ...
                onChange={handleChange}
                className={`w-full p-2 border rounded-lg ${telefoneError ? 'border-red-500' : ''}`}
              />
